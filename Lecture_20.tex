...
\subitem{All solution lie within the span of this set}\vfill
...
\item ~ \student{One eigenvalue is repeated, only one eigenvector}
}
}
...
\vfill
Convert to purely real solution using Euler's identity.
}
}

\slide[Conversion to real-valued vector solutions:  $\lambda_{1,2}=r\pm i\omega $]{\vspace{-1em}
Suppose you have \vspace{-2em}
\[\vec{x}(t) = c_1\overbrace{e^{(r+i\omega)t}(\vec{a}+i\vec{b})}^{\vec{x}_1(t)} + c_2\overbrace{e^{(r-i\omega)t}(\vec{a}-i\vec{b})}^{\vec{x}_2(t)}\]
\vspace{-1.9em}

Then the first eigensolution can be written as
\student{
\algn{\vec{x}_1(t) &= e^{r t}(\cos(\omega t) +i\sin(\omega t)) (\vec{a}+i\vec{b}) \\
&=e^{r t}\left[ (\cos(\omega t)\vec{a}-\sin(\omega t)\vec{b})  + i (\sin(\omega t )\vec{a} + \cos(\omega t ) \vec{b}  )\right]\\
&={\rm Re}\left(\vec{x}_1\right) + i {\rm Im}\left(\vec{x}_1\right) \quad\text{and also } \vec{x}_2 = {\rm Re}\left(\vec{x}_1\right) - i {\rm Im}\left(\vec{x}_1\right)  }
}Choose $c_1$ and $c_2$ as complex conjugate, then 
\student{\algn{\vec{x}(t) &= c_1\vec{x}_1 + c_2 \vec{x_2} = (c_1+c_2) {\rm Re}\left(\vec{x}_1\right) + i (c_1-c_2) {\rm Im}\left(\vec{x}_1\right) \intertext{with $c_1+c_2=d_1\in\mathbb{R}$  and  $c_1-c_2=-id_2$ where $d_2\in\mathbb{R}$}
\vec{x}(t) &= \underbrace{d_1 {\rm Re}(\vec{x}_1(t))}_{\text{Real}}+ \underbrace{d_2 {\rm Im}(\vec{x}_1(t))}_{\text{Real}}
}
}
}

\slide[Find the real-valued solution for the ODE:  \hfill \small $\larray{ \dd{x}{t} = -x + 2y \\ \dd{y}{t} = -2x - y}$]{\vspace{-2em}
\[\vec{x}_1(t) = e^{(-1+2i)t}\mat{c}{1\\ i} \student{= e^{-t}(\cos(2t)+i\sin(2t))\bigg(\underbrace{\mat{c}{1\\0}}_{\vec{a}} +i \underbrace{\mat{c}{0\\1}}_{\vec{b}}\bigg)}\] 
\vspace{-.5em}
\student{
