...
\slide[Example: Skydiving \hfill\begin{minipage}{0.25\textwidth}{ \includegraphics[height=1cm]{images/airplane.pdf}}\end{minipage} ]{\vspace{-1em}
\begin{center}
...
\student{\algn{a&=v\p\\ \Aboxed{ mv\p&=-mg-\mu v}}}
}{
\small{2$^{\text{nd}}$ order linear ODE}
\student{\algn{a&=x\pp, \quad v=x\p\\ \Aboxed{mx\pp&=-mg-\mu x\p}}}
}{
...
We will learn different methods to solve these three different DEs.
}
...
\student{\subitem{A DE with derivatives w.r.t. only one independent variable. \vfill\item \ex{} $\dd{y}{t} = y(t) + 3 \qquad \text{or} \qquad \dd{y}{t} = \sin(y) + \cos(t) $ }}\vfill
...
\item \ex{Temperature of a metal rod} \\~\\ Heat/Diffusion eq: $\pd{u}{t} = D\pdn{u}{x}{2}$ \hfill - \hfill 2$^{\text{nd}}$ order linear PDE\vfill
