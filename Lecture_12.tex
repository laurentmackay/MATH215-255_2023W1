...
x_p\pp&=-\omega^2(A\cos (\omega t) +B\sin(\omega t))}
\[-\omega^2(A\cos (\omega t)+B\sin(\omega t))+\omega_0^2(A\cos (\omega t)+B\sin(\omega t)) = \frac{F_0}{m}\cos (\omega t)\]
\algn{\uline{\sin(t):}\;\;-\omega^2B+\omega_0^2B &=0&\Rightarrow B=&0\\
-\omega^2A+\omega_0^2A &= \frac{F_0}{m} &\Rightarrow A=&\frac{F_0}{m\left(\omega_0^2-\omega^2 \right)}\\
...
\vfill
\uline{MATLAB HW3:} Find the amplitude of motion numerically.
}\vfill
...
\uline{MATLAB HW3:}\\~\\Find $\omega_{\rm max}$ numerically.}
}
}
}
...
\centerline{This phenomenon is called quasi-resonance.}\vfill
\centerline{$\omega_{\rm max}$ is called the practical resonance frequency}
}
\vfill
}
